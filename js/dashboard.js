...
function showSection(sectionId) {
    ...
    if (sectionId === 'memories') {
        ...
        // Pausar música na seção de memórias
...
function openModal(modalId) {
    ...
    if (modalId === 'birthdayModal') {
        startConfetti();
             // Tocar música especial no modal de aniversário
...
function createFallingFlowers() {
    ...
    container.className = 'falling-flowers-container';
    ...
    for (let i = 0; i < 15; i++) {
        setTimeout(() => {
            ...
            flower.style.animation = `falling ${Math.random() * 10 + 5}s linear infinite`;
...
function setupPhotoHoverEffect() {
    ...
    photos.forEach(photo => {
        photo.addEventListener('mouseenter', function() {
            ...
            // Adicionar pequeno efeito de brilho
...
// Mensagens Especiais Aleatórias
...
function animateCards() {
    const cards = document.querySelectorAll('.message-card, .wish-card, .timeline-item');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Aguardar jQuery carregar antes de inicializar carousel
    ...
    // Funcionalidades originais (sem música)
